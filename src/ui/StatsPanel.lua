...
local function build()
  ...
  panel = W.panel(UIParent, 360, 240 + nAch * 17, "Your Poker Record", true)
  ...
  panel.achHead = W.label(panel, "Achievements", "GameFontNormal", "LEFT")
...
function ns.UI.showStats()
  ...
  panel.hands:SetText(("Hands: %s played, %s won (%d%%) — best streak %s"):format(
    W.commas(hands), W.commas(wins), rate, W.commas(s.bestStreak or 0)))
  local net = s.net or 0
  panel.net:SetText("Net chips: " .. (net >= 0 and ("|cff40d940+" .. W.commas(net) .. "|r")
    or ("|cffff5555-" .. W.commas(-net) .. "|r")))
  panel.big:SetText("Biggest single-hand win: " .. W.commas(s.biggestWin or 0))
  panel.best:SetText("Best hand made: " .. (s.bestHandName or "—"))
  panel.bluff:SetText(("Showdowns won: %s   ·   uncontested (bluff) wins: %s"):format(
    W.commas(s.showdownWins or 0), W.commas(s.bluffWins or 0)))
  panel.sng:SetText(("Sit & Gos: %s played, %s won%s"):format(
    W.commas(s.sngPlayed or 0), W.commas(s.sngWon or 0),
    s.sngBest and ("  (best finish: " .. s.sngBest .. ")") or ""))
  panel.meta:SetText(("Hands dealt as host: %s   ·   hands verified clean: %s"):format(
